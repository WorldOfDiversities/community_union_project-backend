...
- AWS_S3_ENDPOINT_URL (for Supabase Storage use `https://<project-ref>.supabase.co/storage/v1/s3`)
- AWS_S3_REGION_NAME (optional)

Render note:

- Media uploads should go to a persistent object store. Render's local filesystem is not durable across deploys, so gallery images and profile pictures will 404 if they are saved there.
